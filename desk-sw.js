...
var 이름 = 'desk-v5';   // desk.html 을 고쳐 올릴 때마다 숫자를 올린다 — 안 올리면 폰은 옛 화면을 계속 연다(09-18)
...
self.addEventListener('notificationclick', function (e) {
  ...
  e.waitUntil(self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (cs) {
    ...
    return 데스크[0].focus().then(function (wc) {
      if (wc && (wc.focused || wc.visibilityState === 'visible')) return 기록(d, '알림눌림', '있던 창 앞으로 ok · ' + wc.visibilityState);
      return 열기('앞으로 했는데 안 보임 · ' + (wc ? wc.visibilityState : 'null'));
