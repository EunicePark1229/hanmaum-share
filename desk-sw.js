...
var 이름 = 'desk-v45';   // desk.html 을 고쳐 올릴 때마다 숫자를 올린다 — 안 올리면 폰은 옛 화면을 계속 연다(09-18)
...
self.addEventListener('notificationclick', function (e) {
  ...
  // 2026-09-24 유니스 「알림을 클릭하면 그 화면으로 이동할 수 있도록」 — 탭만이 아니라 그 줄까지 연다.
  var 항목 = d.항목 || '';
  var 주소 = new URL('./desk.html?열림=알림&탭=' + encodeURIComponent(d.탭 || '홈') +
                   (항목 ? '&항목=' + encodeURIComponent(항목) : ''), self.registration.scope).href;   // 앱이 「알림으로 열렸다」를 기록에 남기게 — 손으로 연 것과 구별(09-18 15:0x)
  ...
  e.waitUntil(self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function (cs) {
    ...
    // 창이 이미 떠 있으면 앞으로 가져오는 것만으로는 보던 화면 그대로다 — 어디로 갈지 알려 준다.
    if (보이는.length && 'focus' in 보이는[0]) return 보이는[0].focus().then(function (c) {
      try { (c || 보이는[0]).postMessage({ 종류: '알림열기', 탭: d.탭 || '홈', 항목: 항목 }); } catch (e) {}
      return 기록(d, '알림눌림', '보이는 창 앞으로 → ' + (항목 || d.탭 || '홈'));
